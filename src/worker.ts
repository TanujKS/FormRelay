...
type ErrorSummary = {
  ...
  function reportOperationalError(...args: unknown[]): void {
    console.error(...args);
  }
  ...
  async function notifyDiscordAboutError(
    ...
    if (!env.DISCORD_ERROR_WEBHOOK) {
      reportOperationalError("[Discord] DISCORD_ERROR_WEBHOOK is not configured");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        reportOperationalError(`[Discord] Webhook failed: ${response.status} ${response.statusText}`, responseText);
      }
    } catch (notifyError) {
      reportOperationalError("[Discord] Webhook request failed:", normalizeError(notifyError));
    }
  }
  ...
  export default {
    async fetch(req: Request, env: Env, ctx: ExecutionContext) {
      ...
      const isTestErrorRoute = url.pathname === "/test-error";
      ...
      if (!isFormRoute && !isTestErrorRoute) {
        debugLog(env, `404: Path not found: ${url.pathname}`);
        return new Response("Not Found", { status: 404 });
      }
  
      if (!isTestErrorRoute && req.method !== "POST") {
      ...
      try {
        if (isTestErrorRoute) {
          throw new Error("Intentional test error triggered by /test-error");
        }
        ...
        if (isDevMode) {
          debugLog(env, "[DEV_MODE] Enabled: overriding form-specific recipients with defaultNotifyTo.");
          debugLog(env, "[DEV_MODE] Original form notifyTo:", formConfig?.notifyTo || []);
          debugLog(env, "[DEV_MODE] Final recipients used:", recipients);
        }
        ...
        const error = normalizeError(err);
        reportOperationalError("❌ Form submission failed:", error);
...
`;
  ...
  async function sendWithMailgun(env: Env, { to, from, subject, html, text }: { to: string; from: string; subject: string; html: string; text: string; }) {
    ...
    if (devMode) {
      debugLog(env, "[DEV_MODE] Mailgun email debug payload:", {
        to,
        from,
        subject,
        mailgunUrl: url,
